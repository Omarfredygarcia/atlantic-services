...
export default function TiendasPage() {
  ...
  async function guardar() {
    ...
    const supabase = createClient()
    if (editing) {
      const { error } = await supabase.from('tiendas').update(form).eq('id', editing.id)
      setMsg(error ? `❌ ${error.message}` : '✅ Tienda actualizada')
    } else {
      const { error } = await supabase.from('tiendas').insert([form])
      setMsg(error ? `❌ ${error.message}` : '✅ Tienda agregada')
    }
    setSaving(false)
    cerrarModal()
    cargar()
  ...
  async function toggleActivo(item: Tienda) {
    const supabase = createClient()
    await supabase.from('tiendas').update({ activo: !item.activo }).eq('id', item.id)
    cargar()
  }
    ...
    <div className="min-h-screen bg-[#141414]">
      <nav className="bg-[#1C1C1C] border-b border-[#333] px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-white font-bold text-sm">ATLANTIC SERVICES — Tiendas</span>
        </div>
      ...
      <div className="p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-gray-500 text-sm ml-auto">{items.length} tiendas</span>
        </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={cerrarModal}>
          <div className="bg-[#1C1C1C] border border-[#333] rounded-xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <h2 className="text-white font-bold text-lg mb-5">{editing ? '✏️ Editar Tienda' : '+ Nueva Tienda'}</h2>
            <div className="flex flex-col gap-4">
              ...
              <div>
                <label className="text-gray-400 text-xs mb-1 block">URL del sitio web</label>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-[#1C1C1C] border border-red-700 rounded-xl p-6 w-full max-w-sm">
            <h2 className="text-white font-bold text-lg mb-2">¿Eliminar tienda?</h2>
